...
class SwiftPopTipView: UIView {
    ...
    var cornerRadiuss: CGFloat = 10
    var borderWidths: CGFloat = 0.1
    var highlight = false
    var hasShadow: Bool {
        get {
    ...
    func contentFrame() -> CGRect {
        ...
        let contentFrame = CGRect(x: customBubbleFrame.origin.x + cornerRadiuss,
                                  y: customBubbleFrame.origin.y + cornerRadiuss,
                                  width: customBubbleFrame.size.width - cornerRadiuss*2,
                                  height: customBubbleFrame.size.height - cornerRadiuss*2)
        return contentFrame
    }
    ...
    override func draw(_ rect: CGRect) {
        ...
        if pointDirection == .up {
            ...
            bubblePath.addArc(tangent1End: CGPoint(x: bubbleRect.origin.x+bubbleRect.size.width, y: bubbleRect.origin.y),
                              tangent2End: CGPoint(x: bubbleRect.origin.x+bubbleRect.size.width, y: bubbleRect.origin.y+cornerRadiuss),
                              radius: cornerRadiuss)
            bubblePath.addArc(tangent1End: CGPoint(x: bubbleRect.origin.x+bubbleRect.size.width, y: bubbleRect.origin.y+bubbleRect.size.height),
                              tangent2End: CGPoint(x: bubbleRect.origin.x+bubbleRect.size.width-cornerRadiuss, y: bubbleRect.origin.y+bubbleRect.size.height),
                              radius: cornerRadiuss)
            bubblePath.addArc(tangent1End: CGPoint(x: bubbleRect.origin.x, y: bubbleRect.origin.y+bubbleRect.size.height),
                              tangent2End: CGPoint(x: bubbleRect.origin.x, y: bubbleRect.origin.y+bubbleRect.size.height-cornerRadiuss),
                              radius: cornerRadiuss)
            bubblePath.addArc(tangent1End: CGPoint(x: bubbleRect.origin.x, y: bubbleRect.origin.y),
                              tangent2End: CGPoint(x: bubbleRect.origin.x+cornerRadiuss, y: bubbleRect.origin.y),
                              radius: cornerRadiuss)
            bubblePath.addLine(to: CGPoint(x: targetPoint.x+sidePadding-pointerSize, y: targetPoint.y+pointerSize))
        } else {
            bubblePath.move(to: CGPoint(x: targetPoint.x+sidePadding, y: targetPoint.y))
        ...
        if borderWidths > 0 {
            let numBorderComponents: size_t = borderColors.cgColor.numberOfComponents
            let borderComponents = borderColors.cgColor.components
            var r, g, b, a: CGFloat
        ...
        if let title = title {
            ...
            title.draw(with: titleFrame, options: .usesLineFragmentOrigin, attributes: [NSAttributedStringKey.font : titleFont, NSAttributedStringKey.foregroundColor : titleColor!, NSAttributedStringKey.paragraphStyle : titleParagraphStyle], context: nil)
        }
        ...
        if let message = message {
            ...
            if let title = title {
                ...
                titleParagraphStyle.alignment = titleAlignment
                titleParagraphStyle.lineBreakMode = .byClipping
                textFrame.origin.y += title.boundingRect(with: CGSize(width: textFrame.size.width, height: 99999.0), options: .usesLineFragmentOrigin, attributes: [NSAttributedStringKey.font : titleFont, NSAttributedStringKey.paragraphStyle : titleParagraphStyle], context: nil).size.height
            }
            ...
            message.draw(with: textFrame, options: .usesLineFragmentOrigin, attributes: [NSAttributedStringKey.font : textFont, NSAttributedStringKey.paragraphStyle : textParagraphStyle, NSAttributedStringKey.foregroundColor : textColor], context: nil)
        }
    }
    ...
    func presentPointingAtView(_ targetView: UIView, containerView: UIView, animated: Bool) {
        ...
        if let message = message {
            ...
            textParagraphStyle.alignment = textAlignment
            textParagraphStyle.lineBreakMode = .byWordWrapping
            textSize = message.boundingRect(with: CGSize(width: rectWidth, height: 99999.0), options: .usesLineFragmentOrigin, attributes: [NSAttributedStringKey.font: textFont, NSAttributedStringKey.paragraphStyle : textParagraphStyle], context: nil).size
        }
        if let customView = customView {
        ...
        if let title = title {
            let titleParagraphStyle = NSMutableParagraphStyle()
            titleParagraphStyle.lineBreakMode = .byClipping
            textSize.height += title.boundingRect(with: CGSize(width: rectWidth, height: 99999.0), options: .usesLineFragmentOrigin, attributes: [NSAttributedStringKey.font : titleFont, NSAttributedStringKey.paragraphStyle: titleParagraphStyle], context: nil).size.height
        }
        
        bubbleSize = CGSize(width: textSize.width + cornerRadiuss*2, height: textSize.height + cornerRadiuss*2)
        ...
        if x_p - pointerSize < x_b + cornerRadiuss {
            x_p = x_b + cornerRadiuss + pointerSize
        }
        if x_p + pointerSize > x_b + bubbleSize.width - cornerRadiuss {
            x_p = x_b + bubbleSize.width - cornerRadiuss - pointerSize
        }
